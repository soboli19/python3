...
# Hiding ping comand output and possible errors
        if os.system("ping -c 1 {} >/dev/null 2>&1".format(hostname)) is 0 and mypassword.decode() != "ERROR - INVALID DATA OR SCRIPT ACCESS" :
...
# Bilding commands for updating infrasrv DNS configuration files
         command6 = """if [ $(cat /etc/resolv.conf | grep -e 172.xxx.xxx.xxx -e 172.xxx.xxx.xxx | wc -l) -eq 2 ]; then echo 'resolv.conf is good'; else echo 'Fixing resolv.conf'; sed -i.bkp 's/172\.xxx\.xxx\.xxx/172.xxx.xxx.xxx/g;s/172\.xxx\.xxx\.xxx/172.xxx.xxx.xxx/g' /etc/resolv.conf ; fi;
...
if [ $(cat /etc/named.conf |grep allow-recursion | wc -w) -lt 8 ]; then echo 'some entries in allow-recursion missing fixing problem'; sed -i '11 s/192.168.0.0\/16;/& 10.0.0.0\/8; 172.0.0.0\/8; /' /etc/named.conf; else echo 'allow-recursion is good'; fi; if [ $(cat /etc/named.conf |awk '/allow-update { 172.21.xxx.xxx/{print}'| awk 'FNR==3 {print}'| wc -w) -lt 7 ]; """
